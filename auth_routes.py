...
def func(user1:LoginModel,access_token:str):
    user_to_update = session.query(User).filter(User.username==user1.username).first()
...
async def login(user:LoginModel,Authorize:AuthJWT=Depends()):
    ...
    if db_user and check_password_hash(db_user.password,user.password):
        access_token=Authorize.create_access_token(subject=db_user.username,expires_time=timedelta(days=1))
        refresh_token=Authorize.create_refresh_token(subject=db_user.username,expires_time=timedelta(days=1))
